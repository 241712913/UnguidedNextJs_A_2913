...
export default function DashboardPage() {
    ...
    return (
      <div className="bg-[#F3F5F4] min-h-screen">
        ...
        <div className="p-3 space-y-4 animate-pulse">
          ...
          <div className="bg-gradient-to-r from-emerald-700 via-emerald-600 to-emerald-500 text-white p-6 rounded-[30px] shadow-md flex flex-col sm:flex-row justify-between gap-4">
            <div>
              <div className="h-8 w-60 bg-white/25 rounded-xl" />
              <div className="h-4 w-40 bg-white/15 rounded mt-3" />
            </div>
            <div className="sm:text-right">
              <div className="h-4 w-32 bg-white/15 rounded ml-auto" />
              <div className="h-10 w-52 bg-white/25 rounded-xl mt-3 ml-auto" />
              <div className="h-3 w-40 bg-white/10 rounded mt-3 ml-auto" />
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1,2,3,4].map((item) => (
              <div key={item} className="bg-white rounded-[28px] p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
              ...
              <div className="h-64 flex items-end justify-between gap-3">
                {[1,2,3,4,5,6,7].map((bar) => (
                  <div key={bar} className="flex-1 flex flex-col items-center">
                    <div className="h-3 w-6 bg-gray-200 rounded mb-2" />
                    <div className="w-full max-w-[45px] bg-gray-200 rounded-t-[20px]" style={{ height: `${40+bar*18}px` }} />
                    <div className="h-3 w-8 bg-gray-100 rounded mt-3" />
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="bg-[#F3F5F4] min-h-screen">
      ...
      {/* HEADER */}
      <div className="bg-gradient-to-r from-emerald-700 via-emerald-600 to-emerald-500 border border-white/10 text-white p-6 m-3 mt-5 rounded-[30px] shadow-md flex flex-col sm:flex-row justify-between gap-4">
        <div>
          ...
          <div className="h-64 flex items-end justify-between gap-3">
            {trenData.map((item, i) => (
              <div key={i} className="flex-1 flex flex-col items-center">
                ...
                <div
                  className={`w-full max-w-[45px] rounded-t-[20px] hover:scale-105 transition-all duration-500 ${
                    item.total > 0
                ...
                />
                <span className="text-[10px] text-gray-500 mt-3 text-center leading-tight">
                  {item.tanggal}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-5">
            {data.layanan.map((item: any, i: number) => {
              ...
              return (
                <div key={i}>
                  <div className="flex justify-between mb-2">
                    <span className="text-sm text-gray-700 font-medium">{item.layanan}</span>
                    <span className="text-sm font-semibold text-emerald-700">{Math.round(persen)}%</span>
                  </div>
                  <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
                    <div
          ...
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="relative flex items-center">
              <input
                ...
                className="bg-[#F6F7F7] border border-[#D9E1DD] rounded-2xl px-4 py-3 pr-10 text-sm outline-none"
              />
              {tanggalFilter !== "all" && (
                <button
            ...
            <a
              href="/api/laporan"
              className="bg-gradient-to-r from-emerald-700 to-emerald-600 hover:opacity-90 text-white px-5 py-3 rounded-2xl text-sm font-medium shadow-[0_10px_30px_rgba(0,0,0,0.08)] transition-all duration-300 text-center"
            >
              Download Laporan
            </a>
          </div>
